...
The given option is descriped as followed: 
...
#' @examples
#' up_and_in_put_option_R(126,100,95,0.24,0.07,0.75,125,1000)
```

## Determing the Barrier
```{r Barrier, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
# R wrapping function: general
getEuropeanPutPrice_withB <- function(B) {
  return(optionPricer::getEuropeanPutPrice(252,100,95,0.24,0.07,0.75,B,10000))
}
...
```{r Price Estimation, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
price <- getEuropeanPutPrice_R(252,100,95,0.24,0.07,0.75,125,10000)

```

The theoretical option price is estimated as **`r price`** for the given option. 

## Investigate Relationship of Price, Volatility and Time to Maturity 
```{r Relationship, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}

# R wrapping function: option price vs. volatility and maturity
getEuropeanPutPrice_WithVolAndExpiry <- function (vol, expiry) {
  return(
    optionPricer::getEuropeanPutPrice(252,100,95,vol,0.07,expiry,125,10000))
}
...
1. **time to maturity** -- influences positively the put option values. The longer time to maturity, the higher the probability that the option becomes active and in-the-money.
